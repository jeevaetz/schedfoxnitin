...
import rmischeduleserver.mysqlconnectivity.queries.client.client_search_query;
/**
 *
 * @author  Ira Juneau
 */
public class ClientSearch extends javax.swing.JDialog {
    ...
    public ClientSearch(java.awt.Frame parent, boolean modal, Connection parentConn, xClientEdit myParent) {
        super(parent, modal);
        ...
        myReturnedEmployees.addColumn("Client Name", List_View.STRING);
    ...
    private class mySelectionListener extends MouseAdapter {
        ...
        public void mouseClicked(MouseEvent e) {
            if (((String)myReturnedEmployees.getTrueValueAt(myReturnedEmployees.getSelectedRow(), 3)).equals("1")) {
                myparent.setDeleted("1");
            } else {
                myparent.setDeleted("0");
            }
            ...
            Main_Window.getClientEditWindow().setSelected((String)myReturnedEmployees.getTrueValueAt(myReturnedEmployees.getSelectedRow(), 0));
        }
    }
    ...
    private void initComponents() {
        ...
        firstTxt.setMaximumSize(new java.awt.Dimension(600, 28));
        ...
        jLabel1.setText("Double click on an clients name to bring up the client.");
        jPanel2.add(jLabel1);
    ...
    private void FindEmpsActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_FindEmpsActionPerformed
        client_search_query searchQuery = new client_search_query();
        ...
        searchQuery.setPreparedStatement("%" + myfname + "%", myfname, myphone, myphone, "%" + myemail + "%", myemail, "%" + mycity + "%", mycity, "%" + myzip + "%", myzip);
        ...
        for (int i = 0; i < myListOfCompanies.size(); i++) {
            ...
            try {
                rs = myConnection.executeQuery(searchQuery);
            } catch (Exception e) {}
            
            for (int x = 0; x < rs.length(); x++) {
                String[] myVals = new String[7];
                myVals[0] = rs.getString("client_id");
                myVals[1] = myConnection.myCompany;
                myVals[2] = rs.getString("branch_id");
                myVals[3] = rs.getString("client_is_deleted");
                ...
                myVals[6] = rs.getString("client_name");
